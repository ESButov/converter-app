...
describe('ClrPage', () => {
  ...
  it('calculates drug volumes after species and weight entry', async () => {
    ...
    expect(screen.getAllByText('Разведение: 1 мл препарата 1 мг/мл + 9 мл 0.9% NaCl').length).toBeGreaterThan(1)
  ...
  it('shows special epinephrine dilution for exotic animals under 1 kg', async () => {
    ...
    expect(screen.getByText('Разведение: 0.1 мл адреналина 1 мг/мл + 9.9 мл 0.9% NaCl')).toBeTruthy()
    expect(screen.getByText('Разведение: 0.1 мл атропина 1 мг/мл + 9.9 мл 0.9% NaCl')).toBeTruthy()
    expect(screen.getByText('1.2 мл-1.62 мл')).toBeTruthy()
